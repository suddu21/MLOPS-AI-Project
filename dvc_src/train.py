...
logging.basicConfig(level=logging.INFO,
                    handlers=[logging.FileHandler(r"C:\Users\sudha\Desktop\My-Data\Education\IITM\2nd-sem\ML-Ops-Lab\MLOPS-AI-Project\dvc_src\logs/train.log"), logging.StreamHandler()]
)
...
MODEL_PATH = r'C:\Users\sudha\Desktop\My-Data\Education\IITM\2nd-sem\ML-Ops-Lab\MLOPS-AI-Project\dvc_src\models/fraud_model.pkl'
...
def train_model():
    ...
    try:
        ...
        logging.info(f"Transformed dataset loaded successfully. Shape: {df.shape}")
        ...
        logging.info("Loading pre-saved test set...")
        test_df = pd.read_csv(TEST_DATA_PATH)
        #print(f"Test set loaded successfully. Shape: {test_df.shape}")
        logging.info(f"Test set loaded successfully. Shape: {test_df.shape}")
        #print(f"Test set Class 1 count: {test_df['Class'].sum()}")
        logging.info(f"Test set Class 1 count: {test_df['Class'].sum()}")
        ...
        for params, mean_score, std_score in zip(
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                logging.info(f"Cross-validation F1-score: {mean_score:.4f} (+/- {std_score:.4f})")
                ...
                logging.info("Evaluating on pre-saved test set...")
                ...
                logging.info("\nValidation Metrics:")
                logging.info(f"Accuracy: {val_accuracy:.4f}, Precision: {val_precision:.4f}, Recall: {val_recall:.4f}, F1: {val_f1:.4f}, AUC: {val_auc:.4f}, Balanced Accuracy: {val_balanced_accuracy:.4f}")
                logging.info("\nTest Metrics:")
                logging.info(f"Accuracy: {test_accuracy:.4f}, Precision: {test_precision:.4f}, Recall: {test_recall:.4f}, F1: {test_f1:.4f}, AUC: {test_auc:.4f}, Balanced Accuracy: {test_balanced_accuracy:.4f}")
